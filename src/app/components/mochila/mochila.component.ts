...
public leer(){
  ...
  if(texto != ""){
    ...
    this.tamMochila = +listaTexto[0];
    (<HTMLInputElement>document.getElementById("spinCantidad")).value = this.tamMochila.toString();
    this.cantObjetos = +listaTexto[1];
    for(var i=2;i<this.cantObjetos+2;i++){
      this.listaObjetos.push(listaTexto[i]);
      this.listaPesos.push(+listaTexto[i+this.cantObjetos]);
      this.listaValores.push(+listaTexto[i+this.cantObjetos*2]);
      this.listaCantidad.push(+listaTexto[i+this.cantObjetos*3]);
    }
  }else{
    this.cond = true;
  }
